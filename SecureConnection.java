...
public class SecureConnection {
    ...
    public byte[] establishSecureConnection() throws IOException {
        try {
            byte[] serverPrivateKey = ECDHKeyExchange.generatePrivateKey(); //Nom nom nom (because it's a byte, like a bite, so nom nom nom)
            byte[] serverPublicKey = ECDHKeyExchange.generatePublicKey(serverPrivateKey); //Maybe I should get diagnosed for autism, I'm pretty sure I'd pass
            ...
            int clientKeyLength = dataIn.readInt(); //Sexy key
            byte[] clientPublicKey = new byte[clientKeyLength]; //I'm losing my mind
            dataIn.readFully(clientPublicKey); //Oh, I forgot that I should also be using these comments to actually document what the stuff does

            byte[] encryptionKey = ECDHKeyExchange.performECDHKeyExchange(serverPrivateKey, clientPublicKey); //This method does the encryption handshake stuff
